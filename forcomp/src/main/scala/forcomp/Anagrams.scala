...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = (for {
    charOcc <- w.toLowerCase.toCharArray.groupBy(c => c).toList
  } yield (charOcc._1, charOcc._2.length)).sortBy(_._1)
  ...
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences.find(_._2.contains(word)).getOrElse((Nil, Nil))._2
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {val yMap = y.foldLeft(Map[Char, Int]())((acc, e) => acc + (e._1 -> e._2))
